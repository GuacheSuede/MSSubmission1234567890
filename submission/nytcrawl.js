        ...
        for(var i = 0; i < alphabets.length; ++i){
            for(var ii = 0; ii < alphabets.length; ++ii){
                for(var iii = 0; iii < alphabets.length; ++iii){
                    ...
                    var c2 = ""
                    var c3 = ""
                    if(alphabets.charAt(i) != " "){
                        c1 = alphabets.charAt(i)
                    }
                    if(alphabets.charAt(ii) != " "){
                        c2 = alphabets.charAt(ii)
                    }
                    if(alphabets.charAt(iii) != " "){
                        c3 = alphabets.charAt(iii)
                    }
                    var combi = c1 + c2 + c3 

                        
                    // navigate to page
                    await Page.navigate({url: "https://nytimes.com/search?sort=best&query="+combi});
                    await Page.loadEventFired();
                    console.log(combi)

                    // check for page validity
                    amount_results = await Runtime.evaluate({
                        expression: "document.querySelectorAll('"+selector.nyt_results+"').length"
                    })        
                    ...
                    if(amount > 0){
                        for(let i = 2; i <= amount; ++i){
                            ...
                            article_title_res = await Runtime.evaluate({
                                expression: "document.querySelector('"+selector.nyt_page_title.replace("{res}", i)+"').innerText"
                            })
                            article_link_res = await Runtime.evaluate({
                                expression: "document.querySelector('"+selector.nyt_page_link.replace("{res}", i)+"').href"
                            })
                            ...
                            if(article_link_res.result.value != undefined){
                                ...
                                article_text_res = await Runtime.evaluate({
                                    expression: "document.querySelector('"+selector.nyt_text+"').innerText"
                                })
                                ...
                                article_date_res = await Runtime.evaluate({
                                    expression: "document.querySelector('"+selector.nyt_date+"').innerText"
                                })

                                article_date = article_date_res.result.value
                                console.log(article_title, article_link, article_date, article_text)
