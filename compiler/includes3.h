...
string init_code_gen(){
    ...
    i_code +=  "function fetch_funda(ticker, quarter, field, cont){ \n fetch(window.furl + ticker + window.furl2 + quarter + window.furl3 + field + window.url_end, { \n method: \'get\', \n headers:{ \n \'Authorization\': \'Basic '+btoa(\'admin:changeit\'), \n \'Content-Type\': \'application/json\' \n }\n}).then(response => {\nreturn response.json(); \n }).then(data => {";
        i_code += "cont = data\[\"_embedded\"\]\[0\]\[\"field\"\];\n";
    i_code += "}); \n } \n";
...
// lower case == algo, uppercase == ticker, jsut for the distinction thoguth no real benefit
    ...
    while(boost::regex_search(code_line, r_algo_current_m, r_algo_current)){
        ...
        if(std::find(algos.begin(), algos.end(), algoname_unique) == algos.end()) {
            ...
            init_code += "window." + algoname_unique + "_socket.onerror = function (error) { \n \t console.log(error); \n }";
        }
    }
    ...
    while(boost::regex_search(code_line, r_buy_m, r_buy)) {
         //boost::replace_first(code_line, r_buy_m[0].str(), "var buy_order = {\""+ r_buy_m[1].str() + "\" , window." + r_buy_m[1].str() + "_values.slice(-1).pop()}; \n window.buys.push(buy_order);");
        boost::replace_first(code_line, r_buy_m[0].str(), "var bo = [\"" + r_buy_m[1].str() +  "\", window." + r_buy_m[1].str() + "_values.slice(-1).pop()]; \n window.buys.push(bo);");
        boost::smatch empty_match;
        r_buy_m = empty_match;
    }
    ...
    while(boost::regex_search(code_line, r_sell_m, r_sell)) {
        boost::replace_first(code_line, r_sell_m[0].str(), "var so = [\"" + r_sell_m[1].str() +  "\", window." + r_sell_m[1].str() + "_values.slice(-1).pop()]; \n window.sells.push(so);");
        boost::smatch empty_match;
